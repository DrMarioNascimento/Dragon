...
Ao entrar pelo celular, o participante pode assistir à abertura vertical ou pulá-la. A abertura horizontal permanece destinada ao telão. A sirene de mudança de fase e os anúncios narrados são emitidos somente no aparelho do mestre para evitar eco entre vários celulares. Os arquivos canônicos ficam em `audio/`; o anúncio correspondente começa depois da sirene e o pódio recebe uma chamada própria ao final da apuração. `audio/encerramento.mp3` toca depois das acusações e antes da revelação, com **Pular narração** disponível ao mestre. `audio/abertura.mp3` toca antes da encenação: o mestre aperta Começar, os celulares ficam em AGUARDE…, e no fim dela a sirene de nevoeiro anuncia `audio/fase-encenacao.mp3`; na metade desse anúncio a tela de quem abre a encenação destrava. Sob a abertura e sob as três rodadas sensoriais corre um fundo de tempestade (`audio/tempestade-costeira-loop.mp3` e `audio/tempestade-rajada.mp3`), em volume baixo e com entrada e saída suaves.
