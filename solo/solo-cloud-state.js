...
(function(){
  ...
  function snap(){
    ...
    return {phase:state.phase,key:state.key,i:state.i,order:state.order,pick:state.pick,seen:state.seen,facts:state.facts,answers:state.answers,scoreFacts:state.scoreFacts,correct:state.correct,
      atividades:state.atividades,atividadeI:state.atividadeI,sensorPronto:state.sensorPronto,mosaico:state.mosaico,mosaicoPick:state.mosaicoPick,
  ...
  function restore(){
    ...
    if(x.phase&&x.phase!=="home"){
      ...
      state.atividades=Array.isArray(x.atividades)?x.atividades:parAtividades();state.atividadeI=Number(x.atividadeI)||0;state.sensorPronto=!!x.sensorPronto;
