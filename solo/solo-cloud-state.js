...
(function(){
  ...
  function snap(){
    ...
    return {phase:state.phase,key:state.key,i:state.i,order:state.order,pick:state.pick,seen:state.seen,facts:state.facts,answers:state.answers,scoreFacts:state.scoreFacts,correct:state.correct,
      percursoPronto:state.percursoPronto,percursoResultado:state.percursoResultado,atividades:state.atividades,atividadeI:state.atividadeI,sensorPronto:state.sensorPronto,mosaico:state.mosaico,mosaicoPick:state.mosaicoPick,
      mercadoEtapa:state.mercadoEtapa,mercadoEscolhas:state.mercadoEscolhas,contraponto:state.contraponto};
  }
  function save(force){
  ...
  function restore(){
    ...
    if(x.phase&&x.phase!=="home"){
      ...
      state.percursoPronto=!!x.percursoPronto;state.percursoResultado=x.percursoResultado||null;state.atividades=Array.isArray(x.atividades)?x.atividades:parAtividades();state.atividadeI=Number(x.atividadeI)||0;state.sensorPronto=!!x.sensorPronto;
      state.mosaico=Array.isArray(x.mosaico)?x.mosaico:[];state.mosaicoPick=x.mosaicoPick==null?null:Number(x.mosaicoPick);
      state.mercadoEtapa=Number(x.mercadoEtapa)||0;state.mercadoEscolhas=Array.isArray(x.mercadoEscolhas)?x.mercadoEscolhas:[];state.contraponto=x.contraponto==null?null:Number(x.contraponto);
