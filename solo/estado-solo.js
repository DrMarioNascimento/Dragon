...
(function(){
  ...
  function restore(){
    ...
    if(x.phase&&x.phase!=="home"){
      ...
      state.percursoPronto=!!x.percursoPronto;state.percursoResultado=x.percursoResultado||null;state.percursoEtapa=['janela','maquete','vela'].includes(x.percursoEtapa)?x.percursoEtapa:'janela';state.atividades=Array.isArray(x.atividades)?x.atividades:parAtividades();state.atividadeI=Number(x.atividadeI)||0;state.sensorPronto=!!x.sensorPronto;state.sensorTempos=Array.isArray(x.sensorTempos)?x.sensorTempos:[];
