 ...
 * Registra ações observáveis da interface para diagnosticar perda de persistência.
 * Não altera a lógica do jogo. Cada aparelho só cria eventos do próprio uid.
 */
(function(){
'use strict';
const S=window.MosaicoSala;
if(!S||!S.online||!S.db||!S.roomCode||!S.auth)return;
const uid=()=>S.auth.currentUser&&S.auth.currentUser.uid;
const ref=()=>S.db.collection('noite').doc(S.roomCode).collection('eventos');
let seq=0;
function lim(v,n=180){return String(v==null?'':v).slice(0,n)}
async function registrar(tipo,dados={}){
  const u=uid();if(!u)return;
  const id=`${u}_${Date.now()}_${(++seq).toString(36)}`;
  try{
    await ref().doc(id).set({
      ...
      role:lim(S.role||'',20),
      partidaId:lim((S.roomData&&S.roomData.partidaId)||'',40),
      dados
    });
...
document.addEventListener('click',e=>{
  const b=e.target&&e.target.closest&&e.target.closest('button,a,[data-mod]');
  if(!b)return;
  registrar('clique',{
    id:lim(b.id,80),
    texto:lim((b.innerText||b.textContent||'').replace(/\s+/g,' ').trim(),140),
    modulo:lim(b.dataset&&b.dataset.mod,40),
    liberar:lim(b.dataset&&b.dataset.liberar,40)
  });
},true);

document.addEventListener('change',e=>{
  const el=e.target;if(!el)return;
  if(el.matches&&el.matches('[data-c]')) registrar('decisao_campo',{campo:lim(el.dataset.c,80),valor:lim(el.value,180)});
},true);
