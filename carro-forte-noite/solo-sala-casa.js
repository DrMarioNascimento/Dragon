...
(function(){
 ...
 function diario(){
  ...
  if(!eventos.length)return '<p style="color:#8fa3ad;margin:6px 0 0">Nada aconteceu ainda nesta rodada.</p>';
  const hora=t=>new Date(t).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',second:'2-digit'});
  return '<ol style="list-style:none;margin:8px 0 0;padding:0;display:grid;gap:5px">'+eventos.map(e=>`<li style="display:flex;gap:9px;align-items:baseline;padding:6px 9px;border-left:2px solid #d6aa5877;background:#0b1720;border-radius:0 7px 7px 0"><b style="flex:0 0 auto;font:700 10px var(--mono,monospace);color:#8fa3ad">${esc(hora(e.at))}</b><span style="font-size:12.5px;line-height:1.35">${esc(e.texto)}</span></li>`).join('')+'</ol>';
 }
 function render(){
 ...
 window.setInterval(()=>{
  ...
  const alvo=document.getElementById('salaDiario');if(alvo)alvo.innerHTML=diario();
 },1000);
