...
(async function(){
 ...
 const app=appmod.getApps().find(a=>a.name==='dragon-solo-lab')||appmod.initializeApp(cfg,'dragon-solo-lab'),auth=authmod.getAuth(app),db=fs.getFirestore(app);
 if(!auth.currentUser)await authmod.signInAnonymously(auth);
 const ALPH='ABCDEFGHJKLMNPQRSTUVWXYZ23456789';let code='';for(let i=0;i<6;i++)code+=ALPH[Math.floor(Math.random()*ALPH.length)];
 const ref=fs.doc(db,'noite',code);
 await fs.setDoc(ref,{ativa:true,fase:'sala',mestreUid:auth.currentUser.uid,criadaEmMs:Date.now(),ritmo:'automatico',caseId:'carro-forte',ensaio:true});
 window.MOSAICO_ROOM={...(window.MOSAICO_ROOM||{}),local:true,soloLab:true,role:'master',code};
