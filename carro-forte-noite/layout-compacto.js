...
(function () {
  ...
  const brandB = brand.querySelector('b'),
    brandSmall = brand.querySelector('small'),
    mark = brand.querySelector('.mark');
  ...
  status.innerHTML =
    '<div class="action-status-left"><small class="status-vez">VEZ</small><b id="hudTurnMirror">—</b></div><div class="action-status-right"><button id="hudSalaMirror" hidden>SALA</button><span class="hud-pill">⏱ <b id="hudTimerMirror">--:--</b></span><span class="hud-pill">💵 <b id="hudCoinsMirror">--</b></span></div>';
  ...
  [...dock.children]
    ...
    .forEach((b) => buttons.appendChild(b));
  ...
  function mirror() {
    const sourceTurn = document.getElementById('hudTurn'),
      sourceTimer = document.getElementById('hudTimer'),
      sourceCoins = document.getElementById('hudCoins'),
      sourceSala = document.getElementById('hudSala');
    const mTurn = document.getElementById('hudTurnMirror'),
      mTimer = document.getElementById('hudTimerMirror'),
      mCoins = document.getElementById('hudCoinsMirror');
    if (mTurn) mTurn.textContent = sourceTurn?.textContent || '—';
    if (mTimer) mTimer.textContent = sourceTimer?.textContent || '--:--';
    if (mCoins) mCoins.textContent = sourceCoins?.textContent || '--';
    if (salaMirror) salaMirror.hidden = sourceSala?.hidden !== false;
  }
  mirror();
  window.setInterval(mirror, 250);
})();
