...
(function () {
  ...
  function enhance() {
    ...
    if (!lobby) {
      ...
      lobby.id = 'carroLobbyQR';
      lobby.className = 'dr-sala-section';
      const head = panel.querySelector('.dr-sala-head');
      if (head) head.insertAdjacentElement('afterend', lobby);
      else panel.querySelector('.dr-sala-card')?.prepend(lobby);
    }
    const url = joinUrl(code),
      qr = window.MosaicoQR
        ? window.MosaicoQR.svg(url, { nivel: 'M', margem: 4, rotulo: 'QR para entrar na sala' })
        : '';
  ...
  document.addEventListener(
    'click',
    (e) => {
      if (e.target.closest?.('#dragonSalaBtn')) setTimeout(enhance, 60);
    },
    true,
  );
  new MutationObserver(() => {
    if (document.getElementById('dragonSalaPanel')) enhance();
