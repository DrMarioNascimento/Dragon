...
Esta licença abrange as expressões originais materializadas no projeto **MOSAICO — A Verdade é um Fragmento**, em seu repositório de desenvolvimento **Dragon** e em seus componentes, incluindo, sem limitação:

- código-fonte, scripts, estruturas de dados e regras de segurança;
- a redação, seleção, combinação e organização original de mecânicas, sistemas de pontuação e rodadas;
...
É concedida ao público uma autorização limitada, pessoal, revogável, intransferível e não exclusiva, nos limites permitidos pela legislação, para:
...
Essa permissão não inclui copiar o repositório, publicar o caso ou a solução, remover avisos de autoria, capturar ou redistribuir conteúdo substancial, hospedar uma cópia, promover sessão pública ou comercial, nem utilizar os materiais em disciplina, pesquisa, evento, produto ou serviço sem autorização escrita, ressalvadas as hipóteses que a lei autorize independentemente de consentimento.
...
- apresentar o projeto ou qualquer conteúdo original protegido como criação de outra pessoa, grupo ou instituição;
- realizar engenharia reversa fora das hipóteses obrigatoriamente permitidas pela legislação aplicável;
- reproduzir, extrair ou tratar conteúdo protegido para treinamento, ajuste, teste ou avaliação de sistemas de inteligência artificial, inclusive modelos generativos, quando esse uso depender de autorização dos titulares;
- incorporar conteúdo protegido a conjuntos de dados ou índices semânticos destinados a sistemas de inteligência artificial, quando essa incorporação depender de autorização dos titulares.

## 7. Casos, sigilo e experiência dos jogadores

Os casos, soluções, cronologias, personagens, falas e pistas podem constituir expressões narrativas protegidas. A possibilidade técnica de inspecionar arquivos entregues ao navegador não constitui autorização para divulgar ou explorar antecipadamente essas informações fora dos limites desta licença e da legislação aplicável.
...
Quando solicitado ou encerrada a colaboração, a pessoa autorizada deverá cessar o uso e eliminar as cópias sob seu controle, ressalvadas obrigações legais de conservação e demais direitos que não possam ser contratualmente afastados.
