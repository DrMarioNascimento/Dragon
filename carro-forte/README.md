...
Após o relatório da Manhã, o botão **Ir para a Noite** continua a sessão com o mesmo código de sala e a mesma pergunta (`?from=celular&sala=&pergunta=`), e grava um **handoff** (fecho, hipótese, fragmentos revelados) em `noite/{sala}.partida.handoff` — a economia do Captura herda esse fecho pela regra v1.1 + semente de fragmentos (ver `noite/README.md`).
